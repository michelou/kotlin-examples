...
set __PKG_NAME=chapter8.
set _MAIN_NAME=debuggin
...
set __ARGS=-src %_KOTLINT_SOURCE_DIR%
...
call "%_EXE_FILE%"
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to execute Kotlin native "!_EXE_FILE:%_ROOT_DIR%\=!" 1>&2
    set _EXITCODE=1
    goto :eof
)
goto :eof
