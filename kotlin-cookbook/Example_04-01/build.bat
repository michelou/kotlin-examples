...
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to execution Kotlin main class "%_MAIN_CLASS%" 1>&2
    set _EXITCODE=1
    goto :eof
)
goto :eof
...
for /f "usebackq delims=" %%f in (`dir /s /b "%_TEST_CLASSES_DIR%\*Test.class" 2^>NUL`) do (
    ...
    if not !ERRORLEVEL!==0 (
        echo %_ERROR_LABEL% Failed to execute test "!_TEST_MAIN_CLASS!" 1>&"
        set _EXITCODE=1
        goto :eof
    )
    goto :eof
