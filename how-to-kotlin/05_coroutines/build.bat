...
set _TIMER=0
set _VERBOSE=0
set __MAIN_NAME=ThreadsVsCoroutines
set __N=0
:args_loop
set "__ARG=%~1"
...
echo     %__BEG_O%run[:^<name^>]%__END%  execute the generated program ^(default: %__BEG_O%ThreadsVsCoroutines%__END%^)
goto :eof
...
echo -language-version %_LANGUAGE_VERSION% %__NOWARN_OPT% -kotlin-home "%KOTLIN_HOME%" -cp "%__CPATH:\=\\%" -d "%_CLASSES_DIR:\=\\%" > "%__OPTS_FILE%"
...
:run_jvm
set __MAIN_CLASS=%_PKG_NAME%%__MAIN_NAME%Kt
set "__MAIN_CLASS_FILE=%_CLASSES_DIR%\%__MAIN_CLASS:.=\%.class"
