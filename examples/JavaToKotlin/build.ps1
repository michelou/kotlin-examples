...
$CLASSES_JAVA_DIR  = Join-Path -Path $CLASSES_DIR -ChildPath 'main.java'
...
function Compile
{
    ...
    if (Test-Action-Required -FilePath "$TIMESTAMP_FILE" -DirPath "$SOURCE_JAVA_DIR" -Pattern '*.java') {
        Compile-Java
        if ($LASTEXITCODE -ne 0) { Cleanup 1 }
    }
...
function Compile-Java
{
    ...
    $CPATH = $CLASSES_DIR
    #$MODULE_OPTS = "--module-path ""$($Env:JAVA_HOME + $SEP + 'jmod')"""
    $MODULE_OPTS = "--module-source-path ""$($SOURCE_DIR.Replace('\', '\\'))"" --module main.java"
    $JAVAC_OPTS = "-encoding utf8 -source ""$JAVA_SOURCE"" -target ""$JVM_TARGET"" $MODULE_OPTS -classpath ""$($CPATH.Replace('\', '\\'))"" -d ""$($CLASSES_DIR.Replace('\', '\\'))"""

    $OPTS_FILE = Join-Path -Path $TARGET_DIR -ChildPath 'javac_opts.txt'
    [System.IO.File]::WriteAllLines($OPTS_FILE, $JAVAC_OPTS)
...
{
    #$CPATH = $(Build-Classpath) + $CLASSES_DIR
    $CPATH = $CLASSES_JAVA_DIR
    $KOTLINC_OPTS = "-jvm-target ""$JVM_TARGET"" -classpath ""$($CPATH.Replace('\', '\\'))"" -d ""$($CLASSES_DIR.Replace('\', '\\'))"""

    $OPTS_FILE = Join-Path -Path $TARGET_DIR -ChildPath 'kotlinc_opts.txt'
    [System.IO.File]::WriteAllLines($OPTS_FILE, $KOTLINC_OPTS)
...
function Run-JVM
{
    ...
    #$CPATH = $(Build-Classpath) + $CLASSES_DIR
    $CPATH = $CLASSES_JAVA_DIR + $PATH_SEP + $CLASSES_DIR
