...
if %_DEBUG%==1 (
    ...
    echo %_DEBUG_LABEL% Variables  : JAVA_HOME="%JAVA_HOME%" KOTLIN_HOME="%KOTLIN_HOME%"
	echo %_DEBUG_LABEL% Variables  : _JAVA_MAIN_CLASS=%_JAVA_MAIN_CLASS% _KT_MAIN_CLASS=%_KT_MAIN_CLASS% 1>&2
)
...
echo     %__BEG_O%lint%__END%        analyze Java/Kotlin source files with %__BEG_N%KtLint%__END%
...
echo -language-version %_LANGUAGE_VERSION% -Werror -cp "%_CLASSES_DIR:\=\\%" -d "%_CLASSES_DIR:\=\\%" > "%__OPTS_FILE%"
...
echo -language-version %_LANGUAGE_VERSION% -o "%_EXE_FILE:\=\\%" -e "%_PKG_NAME%.main" > "%__OPTS_FILE%"
...
echo -cp "%_KOTLIN_CPATH:\=\\%;%_CLASSES_DIR:\=\\%" -d "%_CLASSES_DIR:\=\\%" > "%__OPTS_FILE%"

if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_JAVAC_CMD%" "@%__OPTS_FILE%" "@%__SOURCES_FILE%" 1>&2
) else if %_VERBOSE%==1 ( echo Compile %__N% Java source files 1>&2
)
...
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_KOTLIN_CMD%" %__KOTLIN_OPTS% %_KT_MAIN_CLASS% 1>&2
) else if %_VERBOSE%==1 ( echo Execute Kotlin main class %_KT_MAIN_CLASS%  1>&2
)
call "%_KOTLIN_CMD%" %__KOTLIN_OPTS% %_KT_MAIN_CLASS%
if not %ERRORLEVEL%==0 (
   set _EXITCODE=1
   goto :eof
)
