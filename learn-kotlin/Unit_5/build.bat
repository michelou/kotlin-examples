...
set _KOTLIN_SOURCE_DIR=%_ROOT_DIR%src\main\kotlin
...
if "%__ARG:~0,1%"=="-" (
    ...
    ) else if /i "%__ARG%"=="-script" ( set _SCRIPT=_script
...
goto :args_loop
:args_done
if %_DEBUG%==1 echo %_DEBUG_LABEL% _CLEAN=%_CLEAN% _COMPILE=%_COMPILE% _EXAMPLE=%_EXAMPLE% _LINT=%_LINT% _RUN=%_RUN% _SCRIPT=%_SCRIPT% _VERBOSE=%_VERBOSE%
...
if "%__EXAMPLE%"=="All" (
    set "__EXAMPLE_FILE=%_KOTLIN_SOURCE_DIR%\Examples.kt"
    set _EXAMPLE=*
) else (
    set "__EXAMPLE_FILE=%_KOTLIN_SOURCE_DIR%\%__EXAMPLE%.kt"
...
echo     -script           execute the Kotlin script
...
echo   Valid names are: all ^(default^), examples
goto :eof
