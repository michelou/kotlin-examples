...
if %_DEBUG%==1 (
    ...
    echo %_DEBUG_LABEL% Variables  : _LANGUAGE_VERSION=%_LANGUAGE_VERSION% _TARGET=%_TARGET% 1>&2
)
...
if "%__EXAMPLE%"=="All" (
    set "__EXAMPLE_FILE=%_MAIN_SOURCE_DIR%\Primitives.kt"
    set _EXAMPLE=*
) else (
    set "__EXAMPLE_FILE=%_MAIN_SOURCE_DIR%\%__EXAMPLE%Example.kt"
...
if exist "%__SOURCES_FILE%" del "%__SOURCES_FILE%" 1>NUL
set __N=0
for /f "delims=" %%i in ('dir /s /b "%_SOURCE_DIR%\main\kotlin\*.kt" 2^>NUL') do (
