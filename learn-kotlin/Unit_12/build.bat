...
if "%__ARG:~0,1%"=="-" (
    ...
    if "%__ARG%"=="clean" ( set _COMMANDS=!_COMMANDS! clean
        ...
        set _COMMANDS=!_COMMANDS! compile
        ...
        if not !_EXITCODE!==0 goto args_done
        set _COMMANDS=!_COMMANDS! compile run
    ) else (
...
) else if %_VERBOSE%==1 ( echo Analyze %__N_FILES% with Ktlint 1>&2
)
call "%_KTLINT_CMD%" %__KTLINT_OPTS% %__SOURCE_FILES%
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to analyze %__N_FILES% with Ktlint 1>&2
    set _EXITCODE=1
    goto :eof
)
goto :eof
...
) else if %_VERBOSE%==1 ( echo Compile %__N_FILES% to directory "!_CLASSES_DIR:%_ROOT_DIR%=!" ^(native^) 1>&2
)
call "%_KOTLINC_NATIVE_CMD%" "@%__OPTS_FILE%" "@%__SOURCES_FILE%"
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to compile %__N_FILES% to directory "!_CLASSES_DIR:%_ROOT_DIR%=!" ^(native^) 1>&2
    set _EXITCODE=1
    goto :eof
)
goto :eof
