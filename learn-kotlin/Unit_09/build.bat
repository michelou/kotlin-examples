...
@rem output parameter: _ARGS_EXAMPLE (comparison, .., when)
:args_example
set _ARGS_EXAMPLE=
set __ARGS=%~1
...
echo.
echo   Valid values: n=comparison,equality,hashcode,if,when ^(default=%__BEG_O%comparison%__END%^)
goto :eof
...
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to execute batch file "%__BATCH_FILE%" 1>&2
    set _EXITCODE=1
    goto :eof
)
...
@rem output parameter: _DOKKA_JAR
:dokka_jar
call :libs_cpath
if not %_EXITCODE%==0 goto :eof

set _DOKKA_JAR=
set "__CPATH=%_DOKKA_CPATH%"
:doc_cpath
for /f "delims=; tokens=1,*" %%f in ("%__CPATH%") do (
    set "__JAR_FILE=%%f"
    if not "!__JAR_FILE:dokka-cli=!"=="!__JAR_FILE!" (
        set "_DOKKA_JAR=!__JAR_FILE!"
    )
    set "__CPATH=%%g"
    goto doc_cpath
)
...
set __DOKKA_ARGS=-outputDir="%_TARGET_DOCS_DIR%" -moduleDisplayName=AAA -sourceSet -src="%_SOURCE_DIR%\main\kotlin\"
...
if not defined __MAIN_CLASS (
    echo %_ERROR_LABEL% Main class "%__MAIN_CLASS%" not found 1>&2
    set _EXITCODE=1
    goto :eof
)
